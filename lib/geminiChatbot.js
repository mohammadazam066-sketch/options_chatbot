/**
 * Gemini Live Conversational AI Engine - Ultra Crisp F&O Trading Advisor
 * Streams 100% Real Live Exchange Market Spot & Option Chain Data for NIFTY, BANKNIFTY, and BSE SENSEX
 */
...
async function processUserQuery(userQuery, symbol = 'NIFTY', userContext = {}) {
  ...
  // FETCH REAL LIVE SPOT PRICE & EXCHANGE METRICS FOR THE DETECTED INDEX
...
/**
 * Live Gemini Call - Ultra Crisp & Short Response System for NIFTY, BANKNIFTY & SENSEX
 */
async function callGeminiAPI(apiKey, query, symbol, userContext, market) {
  ...
  - IF USER ASKS ABOUT SENSEX / NIFTY / BANKNIFTY OR A TRADE: Give EXACT Setup:
...
function generateSmartResponse(originalQuery, queryLower, symbol, userName, market) {
  ...
  if (KNOWLEDGE_BASE.buyingAdvice.some(a => queryLower.includes(a))) {
    ...
    const tradeType = isBull ? `Buy ${symbol} ${Math.round(market.spotPrice / step) * step} CE` : isBear ? `Buy ${symbol} ${Math.round(market.spotPrice / step) * step} PE` : `Bull Call Spread / Wait for breakout`;
