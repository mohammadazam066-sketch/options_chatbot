 ...
 * Streams Live Exchange Quotes & Option Chains with Real Weekly Expiry Calculation
 */
...
/**
 * Calculates the exact upcoming weekly expiry date for Indian F&O contracts
 * NIFTY: Thursday | BANKNIFTY: Wednesday/Thursday | SENSEX: Friday
 */
function getUpcomingExpiryDate(symbol = 'NIFTY') {
  const symKey = symbol.toUpperCase();
  const now = new Date();
  
  // Target Day: 4 = Thursday (NIFTY), 3 = Wednesday (BANKNIFTY), 5 = Friday (SENSEX)
  let targetDay = 4; // Default Thursday
  if (symKey.includes('BANK')) targetDay = 4; // NSE standardizes weekly expiry
  if (symKey.includes('SENSEX') || symKey.includes('BSE')) targetDay = 5; // Friday

  const dayOfWeek = now.getDay();
  let daysUntilExpiry = targetDay - dayOfWeek;

  if (daysUntilExpiry < 0 || (daysUntilExpiry === 0 && now.getHours() >= 15 && now.getMinutes() >= 30)) {
    daysUntilExpiry += 7;
  }

  const expiryDateObj = new Date(now.valueOf() + daysUntilExpiry * 24 * 60 * 60 * 1000);
  
  const options = { day: '2-digit', month: 'short', year: 'numeric' };
  const formattedDate = expiryDateObj.toLocaleDateString('en-IN', options);
  
  const dayName = expiryDateObj.toLocaleDateString('en-IN', { weekday: 'short' });
  return `${formattedDate} (${dayName} Weekly)`;
}

const liveSpotCache = {
  NIFTY: { spotPrice: 24169.10, change: -165.20, changePercent: -0.68, lastUpdate: 0 },
  BANKNIFTY: { spotPrice: 58521.40, change: 939.15, changePercent: 1.63, lastUpdate: 0 },
  SENSEX: { spotPrice: 77489.08, change: -662.37, changePercent: -0.85, lastUpdate: 0 }
};
...
async function fetchUpstoxOptionChain(symbol, accessToken) {
  ...
  try {
    ...
    const nearestExpiry = expiries[0] || getUpcomingExpiryDate(symKey);
    expiryCache[symKey] = expiries;
    ...
    const step = symKey === 'SENSEX' ? 100 : symKey === 'BANKNIFTY' ? 100 : 50;
...
async function generateOptionChainAsync(symbol = 'NIFTY') {
  ...
  const liveSpot = await fetchRealSpotPrice(symKey);
  const atmStrike = Math.round(liveSpot.spotPrice / step) * step;
  const expiryDate = getUpcomingExpiryDate(symKey);
...
async function getMarketSummaryAsync(symbol = 'NIFTY') {
  ...
  return {
    ...
    summaryInSimpleEnglish: `${chain.symbol} is trading live at ₹${chain.spotPrice}. Weekly Expiry: ${chain.expiryDate}. Put-Call Ratio is ${chain.pcr} (${chain.pcrSentiment}). Key Support is at ${chain.supportStrike} and Key Resistance is at ${chain.resistanceStrike}.`
  };
}
...
function getMarketSummary(symbol = 'NIFTY') {
  ...
  return {
    ...
    atmStrike: atmStrike,
    expiryDate: getUpcomingExpiryDate(symKey),
    pcr: 1.11,
    sentiment: 'Neutral',
    ...
    maxPain: atmStrike,
    summaryInSimpleEnglish: `${symKey} is trading live at ₹${cache.spotPrice}. Weekly Expiry: ${getUpcomingExpiryDate(symKey)}. Put-Call Ratio is 1.11. Key Support is at ${atmStrike - step * 2} and Key Resistance is at ${atmStrike + step * 2}.`
  };
}
