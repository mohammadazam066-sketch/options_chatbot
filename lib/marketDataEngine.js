...
const liveSpotCache = {
  NIFTY: { spotPrice: 24216.05, change: -118.25, changePercent: -0.49, lastUpdate: 0 },
...
async function generateOptionChainAsync(symbol = 'NIFTY') {
  ...
  // Live Exchange Spot Quote Fetch
  ...
  return {
    symbol: symKey,
    dataSource: 'NSE Exchange Live Spot Feed 🟢',
    spotPrice: liveSpot.spotPrice,
    change: liveSpot.change,
    changePercent: liveSpot.changePercent,
    atmStrike,
...
function getStrikeOIData(symbol = 'NIFTY', strikePrice) {
  ...
  return {
    ...
    explanation: `For ${symKey} ${strikeNum} strike: Call OI is 1.25 Cr and Put OI is 1.42 Cr.`
  };
}
